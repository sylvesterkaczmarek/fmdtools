...
def test_constant_slice_uses_basic_interval(axis, statistic):
    values = np.array([[0., 10.], [1., 10.], [2., 10.], [3., 10.]])
    if axis != 0:
        values = values.T
    actual = calc_metric_ci(values, method=statistic, axis=axis,
                            n_resamples=499, rng=np.random.default_rng(13))
    expected = bootstrap([values], statistic, axis=axis, method='basic',
                         n_resamples=499, rng=np.random.default_rng(13))
    ...
    constant = 10. if statistic is np.mean else 40.
...
def test_multidimensional_sample_axis(axis):
    ...
    expected = bootstrap([values], np.average, axis=axis, method='basic',
                         n_resamples=499, rng=np.random.default_rng(17))
    np.testing.assert_array_equal(actual[1], expected.confidence_interval.low)
    np.testing.assert_array_equal(actual[2], expected.confidence_interval.high)
...
def test_single_output_axis_does_not_index_another_dimension(axis):
    values = np.arange(5., dtype=float).reshape(5, 1)
    if axis != 0:
        values = values.T
    actual = calc_metric_ci(values, axis=axis, n_resamples=499,
                            rng=np.random.default_rng(19))
    expected = bootstrap([values], np.average, axis=axis,
                         n_resamples=499, rng=np.random.default_rng(19))
    np.testing.assert_array_equal(actual[1], expected.confidence_interval.low)
    np.testing.assert_array_equal(actual[2], expected.confidence_interval.high)
...
def test_all_constant_outputs_follow_return_anyway(axis):
    ...
    for entry in result:
        np.testing.assert_array_equal(entry, [2., 7.])


def test_weighted_data_is_checked_after_preprocessing():
    rates = np.array([1., 2., 4., 8.])
    values = np.column_stack([np.arange(4.), 10. / rates])
    actual = calc_metric_ci(values, rates=rates, n_resamples=499,
                            rng=np.random.default_rng(23))
    weighted = values * rates[:, None]
    expected = bootstrap([weighted], np.average, axis=0, method='basic',
                         n_resamples=499, rng=np.random.default_rng(23))
    np.testing.assert_array_equal(actual[1], expected.confidence_interval.low)
    np.testing.assert_array_equal(actual[2], expected.confidence_interval.high)


def test_history_confidence_bounds_preserve_constant_timesteps():
    hist = History({'a.value': [0., 10.], 'b.value': [1., 10.],
                    'c.value': [2., 10.], 'd.value': [3., 10.],
                    'time': [0., 1.]})
    result = hist.get_mean_ci_errhist('value', n_resamples=499,
                                     rng=np.random.default_rng(29))
    assert result.stat[1] == result.low[1] == result.high[1] == 10.
    assert np.isfinite(result.low).all()
    assert np.isfinite(result.high).all()


@pytest.mark.parametrize('values', [[1., 2., 4., 8.],
                                     [[1., 5.], [2., 6.], [4., 9.], [8., 10.]]])
def test_varying_data_retains_bca_results(values):
    values = np.asarray(values)
    actual = calc_metric_ci(values, n_resamples=499,
                            rng=np.random.default_rng(31))
    expected = bootstrap([values], np.average, axis=0, n_resamples=499,
                         rng=np.random.default_rng(31))
    np.testing.assert_array_equal(actual[1], expected.confidence_interval.low)
    np.testing.assert_array_equal(actual[2], expected.confidence_interval.high)
